...
In this paper, we presented a high-level design and implementation of a memory harvesting technique for tensor management at the GPU level. Our system consists of a client-server architecture, where the client provides the interface to the user and the server is responsible for the physical allocation, prefetching, and eviction of tensors. We discussed the details of the client-server communication using IPC and the offloading process. We also evaluated the performance of our system compared to standard CUDA memory management through benchmarking. Although our system showed slower performance due to the overhead of IPC calls, we observed a linear increase in time taken as the number of tensors increased. 

For future work, we propose exploring a multi-GPU design to further enhance the performance of our system. Overall, our memory harvesting technique provides a promising approach for efficient tensor management in GPU-based systems.
